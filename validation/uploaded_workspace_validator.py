...
class UploadedWorkspaceValidator:
    ...
    def _prepare_uploaded_rows(
        ...
        unexpected_columns = sorted({
            ...
            if column_name not in expected_headers and column_name != entity_column_name
        })
        prepared_rows = [
    ...
    def _normalize_rows_for_comparison(self, rows: list[dict], csv_filename: str) -> Counter:
        """Validate and normalize rows into a Counter for order-independent comparison."""
        model = CsvSchemaHelper.get_model_for_csv_filename(csv_filename)
        headers = CsvSchemaHelper.get_expected_headers_for_csv_filename(csv_filename)
        return Counter(
            self._row_to_counter_key(model(**row).model_dump(), headers)
            for row in rows
        )
    ...
    def _validate_uploaded_table_matches_source(
        ...
        expected_counter = self._normalize_rows_for_comparison(rows=source_rows, csv_filename=csv_filename)
        uploaded_counter = self._normalize_rows_for_comparison(rows=normalized_uploaded_rows, csv_filename=csv_filename)
        if expected_counter != uploaded_counter:
            missing_rows = list((expected_counter - uploaded_counter).elements())[:3]
            unexpected_rows = list((uploaded_counter - expected_counter).elements())[:3]
            failures.append(
                f"{context}: table '{table_name}' contents do not match source CSV '{csv_filename}'. "
                f"Example missing row(s): {missing_rows}; example unexpected row(s): {unexpected_rows}"
            )
    ...
    def _validate_workspace_tables(
        ...
        missing_tables = sorted(set(expected_table_names) - actual_table_names)
        if missing_tables:
            failures.append(
                f"Workspace '{workspace.workspace_name}' is missing expected table(s): {missing_tables}"
            )

        unexpected_tables = sorted(actual_table_names - set(expected_table_names))
