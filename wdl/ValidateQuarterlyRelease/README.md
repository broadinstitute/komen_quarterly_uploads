...
If `include_workspaces` is provided, only sub datasets whose derived workspace name appears in that list are kept. Sub datasets not in the list are skipped before any validation work begins. If a name in the list does not match any sub dataset in the bucket, the script raises an error immediately.

If `exclude_workspaces` is provided, any sub dataset whose derived workspace name appears in that list is skipped. A warning is logged for any name that did not match a sub dataset.
